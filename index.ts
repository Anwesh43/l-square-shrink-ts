...
class DrawingUtil {
    ...
    static drawLSquare(context : CanvasRenderingContext2D, scale : number) {
        ...
        for (var j = 0; j < 2; j++) {
            ...
            for (var k = 0; k < 2; k++) {
                ...
                context.rotate(k * sc2 * Math.PI / 2)
    ...
    static drawLSNode(context : CanvasRenderingContext2D, i : number, scale : number) {
        ...
        DrawingUtil.drawLSquare(context, scale)
    }
}
...
class LSSNode {
    ...
    getNext(dir : number, cb : Function) : LSSNode {
        var curr : LSSNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
